...
public class HITS_HUB extends FS {
    ...
    public void run() throws IOException {
        savefileLog(new Date() + " [INFO] Feature Selection with HITS score: hub inizialized.");
        new File(LoadProperties.MAPPINGPATH+"/FS").mkdirs();
        FileOutputStream fout = new FileOutputStream(LoadProperties.MAPPINGPATH+"/FS/HITS_HUB");
        ...
        savefileLog(new Date() + " [INFO] Feature Selection with HITS score: hub Completed.");
        savefileLog("---------------------------------------------------");
    }
